...
export function getHover(
  ...
  const fn = allFunctions(current, imported).find(f => f.name === word);
  if (fn) {
    return {
      contents: {
...
export function getSignatureHelp(
  ...
  const fn = allFunctions(current, imported).find(f => f.name === call.name);
  if (!fn) return null;
...
export function getDefinition(
  ...
  // Function definition — search current file first, then imported files
  const localFn = current.functions.find(f => f.name === word);
  if (localFn) {
  ...
  for (const imp of current.imports) {
    ...
    if (fn.sourceOffset !== undefined) {
      try {
        ...
        pos = positionInText(text, fn.sourceOffset);
      } catch { /* file not readable, use line 0 */ }
    }
